...
- **netlify.toml** — deploy config: publishes the repo as-is (no build command — `index.html` loads `app/js/game.js` as a native ES module, the browser resolves the rest), plus the `/label` (label queue) and `/label/browse` (Browse) redirects for the admin tools.
