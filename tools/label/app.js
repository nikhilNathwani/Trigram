...
function buildQueue() {
	...
	const unlabeled = S.allTrigrams.filter(
...
function buildWordPanel(wordList) {
	...
	if (!wordList) {
		panel.innerHTML =
			'<div class="word-list"><span class="word-missing">Word list not yet generated. Run <code>tools/scripts/generate_all_word_lists.sh</code></span></div>';
		return panel;
	}
	...
	function showLength(len) {
		$$(".length-tabs button", tabs).forEach((b) =>
...
async function renderQueue() {
	...
	// Comment area
	const commentInput = mk("textarea", {
	...
	async function handleLabel(label) {
		...
		try {
			...
			if (!errEl) {
				errEl = mk("div", { className: "save-error" });
	...
	[
		...
		actions.appendChild(
			mk("button", { className: `action-btn ${cls}`, onclick: handler }, [
				text,
			]),
		);
	});
...
$("#search-input").addEventListener("input", () => {
	...
	if (val.length < 3) {
		$("#search-content").innerHTML =
			'<p class="hint-text">Type a 3-letter trigram above</p>';
		return;
	}
...
async function renderSearchResult(trigram) {
	...
	if (existing?.comment) {
		container.appendChild(
			mk("p", { className: "existing-comment" }, [
				`"${existing.comment}"`,
			]),
		);
	}
	...
	container.appendChild(
		mk("div", { className: "comment-area" }, [commentInput]),
	);
...
async function renderReview() {
	...
	const entries = Object.entries(S.labels)
		...
		.sort((a, b) => {
			const ta =
				a[1].labeledAt?.toDate?.() || a[1].labeledAt || new Date(0);
			const tb =
				b[1].labeledAt?.toDate?.() || b[1].labeledAt || new Date(0);
			return tb - ta;
		});
	...
	if (!entries.length) {
		container.appendChild(
			mk("p", { className: "empty-state" }, [
				filter === "ALL"
					? "No labels yet."
					: `No ${filter} labels yet.`,
			]),
		);
		return;
	}
	...
	entries.forEach(([trigram, data]) => {
		...
		row.appendChild(
			mk(
				"span",
				{ className: `label-badge badge-${data.label.toLowerCase()}` },
				[data.label],
			),
		);
		if (data.comment) {
			row.appendChild(
				mk("span", { className: "review-comment" }, [data.comment]),
			);
		}
...
function renderCalendar() {
	...
	const recentSet = new Set(
		S.calendarData.slice(-5).map((t) => t.toUpperCase()),
	);
	...
	[...S.calendarData].reverse().forEach((trigram, i) => {
		...
		const weekDate = new Date(
			GAME_START.getTime() + (weekNum - 1) * 7 * 24 * 60 * 60 * 1000,
		);
		...
		const row = mk("div", {
			className: `calendar-row${isRecent ? " recent" : ""}`,
		});
		row.appendChild(
			mk("span", { className: "cal-week" }, [`Wk ${weekNum}`]),
		);
		row.appendChild(
			mk("span", { className: "cal-trigram" }, [trigram.toUpperCase()]),
		);
...
function renderStats() {
	...
	[
		...
		const card = mk("div", {
			className: `stat-card${cls ? " stat-" + cls : ""}`,
		});
...
function buildBarChart(title, labels, totalFn, yesFn) {
	...
	labels.forEach((label) => {
		...
		row.appendChild(
			mk("span", { className: "bar-count" }, [`${yes}/${total}`]),
		);
